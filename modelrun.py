...
class ModelRun:
    ...
    # @classmethod
    # def KScheduler(cls, action, deep_sleep=list()):
    #     """
    #     适用于A股的信号采集实时任务
    #     :param deep_sleep:
    ...
    #     :return:
    #     """
    #     # if isinstance(type(ma), modelaction):
    #     #     raise Exception()
    ...
    @classmethod
    def DScheduler(cls, action, start_date=None, execute_date=None, end_date=None, execute_interval=3, tz=None,
                   **kwargs):
        """
        ...
        while 1:
            ...
            # 交易日
                ...
                try:
                    nsds = list()
                    executors = {'default': ThreadPoolExecutor(4), 'processpool': ProcessPoolExecutor(4)}
                    job_defaults = {'coalesce': True, 'max_instances': 1}
                    scheduler = BackgroundScheduler(executors=executors, job_defaults=job_defaults, daemonic=False,
                                                    timezone=tz)
                    if start_date is not None:
                        d = tdy + dt.timedelta(hours=sdt.hour, minutes=sdt.minute, seconds=sdt.second)
                        ...
                        scheduler.add_job(func=action_start, trigger=DateTrigger(d), id='action_start', args=[kwargs])
                    if execute_date is not None:
                        ...
                        for x in xdt:
                            sd = tdy + dt.timedelta(hours=x[0].hour, minutes=x[0].minute, seconds=x[0].second)
                            ed = tdy + dt.timedelta(hours=x[1].hour, minutes=x[1].minute, seconds=x[1].second)
                            if sd > ed:
                                sd = sd + dt.timedelta(days=1)
                                scheduler.add_job(func=action_execute,
                                                  trigger=IntervalTrigger(seconds=execute_interval, start_date=ed,
                                                                          end_date=sd), args=[kwargs])
                            else:
                                scheduler.add_job(func=action_execute,
                                                  trigger=IntervalTrigger(seconds=execute_interval, start_date=sd,
                    ...
                    if end_date is not None:
                        ...
                        nsds.append(d + dt.timedelta(days=1))
                        scheduler.add_job(func=action_end, trigger=DateTrigger(d), id='action_end', args=[kwargs])
                    print(fontcolor.F_GREEN + '-' * 80)
